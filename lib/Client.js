...
class Client extends Homey.SimpleClass {
	...
	async disconnect(){
		if(this._unsubscribeEntities != null){
			try{
				console.log('unsubscribe entity subscription');
				this._unsubscribeEntities();
				await this.sleep(2000);
				this._unsubscribeEntities = null;
			}
			catch(error){
				console.log('Error: close existing connection.');
			}
		}
        ...
        // return new Promise((resolve, reject) => {
			...
			if(address && address != "" 
				...
				try{
					...
					console.log('subscribed to entities and events');
					return true;
				}
				catch(err){ 
					this._connection = null;
						...
						Object.keys(compoundCapabilities).forEach(key => {
							if(compoundCapabilitiesTitles[key] != null) {
								capabilitiesOptions[key] = 
									{
										"title": compoundCapabilitiesTitles[key]
									}
							}
						});
				...
				if(id.startsWith("media_player.")) {
					...
					let media_player = {
						...
						store: {
							...
							sourceList: entity.attributes.source_list,
							soundModeList: entity.attributes.sound_mode_list
						}
					};
